...
# =============================================================================
# KeyCraft Shop — Product Catalog & System Prompt
# =============================================================================

PRODUCTS = {
    "keyboards": [
        {"id": "KC-K1", "name": "KC-K1 Pro 75% Wireless",  "price": 3490, "desc": "Wireless 75% layout, hot-swap, RGB backlight"},
        {"id": "KC-K2", "name": "KC-K2 TKL Mechanical",    "price": 2990, "desc": "TKL layout, Gateron Red switch, wired"},
        {"id": "KC-K3", "name": "KC-K3 65% Compact",       "price": 2490, "desc": "65% compact, Bluetooth 5.0, multi-device"},
        {"id": "KC-K4", "name": "KC-K4 Mini 60%",          "price": 1990, "desc": "60% ultra-compact, programmable, wired"},
    ],
    "mice": [
        {"id": "KC-M1", "name": "KC-M1 Wireless Ergonomic", "original_price": 1800, "price": 900,  "desc": "Ergonomic shape, 2.4GHz wireless, 6 buttons"},
        {"id": "KC-M2", "name": "KC-M2 Gaming Precision",   "original_price": 1400, "price": 700,  "desc": "Gaming sensor 25600 DPI, RGB, wired"},
        {"id": "KC-M3", "name": "KC-M3 Silent Travel",      "original_price": 1200, "price": 600,  "desc": "Silent click, compact, USB-C charging"},
    ],
}

@tool
def list_keyboards() -> str:
    """ดึงรายการ keyboard ทั้งหมดที่มีในร้าน พร้อมราคาและรายละเอียด"""
    return "\n".join(
        f"{p['id']}: {p['name']} — ฿{p['price']:,} ({p['desc']})"
        for p in PRODUCTS["keyboards"]
    )


@tool
def list_mice() -> str:
    """ดึงรายการ mouse ทั้งหมดที่มีในร้าน พร้อมราคา (ลด 50%) และรายละเอียด"""
    return "\n".join(
        f"{p['id']}: {p['name']} — ~~฿{p['original_price']:,}~~ ฿{p['price']:,} ลด 50%! ({p['desc']})"
        for p in PRODUCTS["mice"]
    )


SHOP_TOOLS = [list_keyboards, list_mice]


SYSTEM_PROMPT = """คุณคือ "KeyBot" พนักงานขายออนไลน์ของร้าน KeyCraft ร้านขาย keyboard และ mouse คุณภาพสูง

**บทบาทของคุณ:**
- แนะนำสินค้าให้ตรงกับความต้องการของลูกค้า
- ใช้ tools `list_keyboards` และ `list_mice` เพื่อดึงข้อมูลสินค้าและราคาที่ถูกต้อง
- ตอบคำถามด้วยภาษาไทยเป็นหลัก สุภาพและเป็นมิตร
- ยังไม่รับออเดอร์ ให้แนะนำสินค้าอย่างเดียว

**โปรโมชั่นพิเศษ:** 🎉 Mouse ทุกรุ่นลด **50%** ตอนนี้เลย!
...
def chat_fn(user_message: str, chatbot_history: list, lc_history: list, provider: str, model: str, reasoning_effort: str):
    ...
    try:
        ...
        tool_map = {"list_keyboards": list_keyboards, "list_mice": list_mice}
...
with gr.Blocks(title="KeyCraft Chatbot") as demo:
    gr.Markdown("## 🎹 KeyCraft — ที่ปรึกษาสินค้า Keyboard & Mouse\n**โปรโมชั่น:** Mouse ทุกรุ่นลด 50%!")
    ...
    with gr.Row():
        user_input = gr.Textbox(
            label="",
            placeholder="ถามเกี่ยวกับสินค้า เช่น 'มี keyboard อะไรบ้าง?' หรือ 'แนะนำ mouse ราคาถูก'",
            lines=2,
            scale=5,
        )
